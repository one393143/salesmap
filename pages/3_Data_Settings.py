...
if df is not None:
    ...
    # 獨立的按鈕執行座標解析 (絕對不可自動執行)
    if st.button('執行座標解析'):
        with st.spinner('雲端座標解析中...'):
            # 1. 清洗地址
            temp_df = df.copy()
            if '清洗後地址' not in temp_df.columns:
                temp_df['清洗後地址'] = temp_df[target_col].apply(clean_taiwan_address)
                
            # 2. 地理編碼 (batch_geocode 會自動略過已有座標的行)
            geocoded_df = batch_geocode(temp_df, '清洗後地址', use_api=True)
    ...
    # 如果 session_state 中有資料，顯示結果與下載按鈕
    if st.session_state.get('client_data') is not None:
        ...
        # 確保顯示的欄位存在
        display_cols = [target_col]
        if '清洗後地址' in geocoded_df.columns:
            display_cols.append('清洗後地址')
        if 'Latitude' in geocoded_df.columns:
            display_cols.extend(['Latitude', 'Longitude'])
            
        st.dataframe(geocoded_df[display_cols], use_container_width=True)
